	...
	function init(){
		...
		$.ajax(settings).done(function (response) {
		    ...
		    for (var i = 0; i < Math.min(flights.length, 10); i++) {
		        ...
		        result += "<div class='flight-info-detail'>";
		        result += "<div class='airInfo'>";
		        ...
		        if(flights[i].segments[0].legs[0].destinationStationCode != airport_arrival ){
		        	...
		        	result +=  timeStringDepTo+"<span style='font-size: 0.9em; color: skyblue;'>"+flights[i].segments[0].legs[0].originStationCode +"</span> => "+timeStringArrTo+"<span style='font-size: 0.9em; color:skyblue;'>"+flights[i].segments[0].legs[0].destinationStationCode+ "</span></div>";}
			        ...
			        if(ticketType="ROUND_TRIP" && flights[i].segments[1] != null){
						...
						if(response.data.flights[i].segments[1].legs[0].destinationStationCode != airport_departure ){
							result += timeStringArrToR+"<span style='font-size: 0.9em; color:skyblue;'>"+airport_departure + "</span> (경유 포함)<br></div>";
						}else{
							result += timeStringArrToR+"<span style='font-size: 0.9em; color:skyblue;'>"+response.data.flights[i].segments[1].legs[0].destinationStationCode + "</span><br><div>";
						}
		        ...
		        result += "<div class='airPrice' style='text-align:right; align:right; margin-bottom: 0; margin-top: 5px; padding-top:0; padding-bottom:0;'> 가격: <strong style='font-size: 15pt; text-align:right;' >" + Math.round(flights[i].purchaseLinks[0].totalPrice*1300).toLocaleString() + "원~ </strong> </div>";
		        result += "<input type='button' class='goSite' value='이동하기'  onclick=\"goUrl('" + flights[i].purchaseLinks[0].url + "')\">";
	        	result += "</div>"
		...
		<div style="background-color:#D2EFFF; align:center; text-align:center">
			<div id="flight_header" style="background-color:white">
				<div id="container">
					...
					<input type='button' class="btn-two small rounded"  value="왕복" style="background-color:white; color:black" onclick="toggleFlightType('ROUND_TRIP')">		
				</div>	
				...
				<div id="container_two">
					...
					<span class="btn-two rounded" >
						<img src="/images/flightImages/plane.png" width=30px; height=30px;> 
					...
					<span class="btn-two">
						<img src="/images/flightImages/calenderImg.png" width=30px; height=30px;> <input type="text" id="datepicker" style="border:0">
					</span>
					
					<span class="btn-two" id=dateCome>
						<img src="/images/flightImages/calenderImg.png" width=30px; height=30px;>
						<input type="text" id="datepicker_round" style="border:0">
					</span>
					...
					<span class=flight_set>
						<button type="submit"  class="searchBtn"  onclick="init()">검색하기</button>
					</span>
			...
			<div id="flight_body" style="text-align:center; margin:auto; ">			
